...
Page {
    ...
    Component {
        ...
        Dialog {
            ...
            Label {
		    text: i18n.tr("Your device needs Android 14+ images for Waydroid to work but the currently installed Ubuntu Touch build doesn't support them. <br><br> Please upgrade to 24.04-2.1 or above (alternatively switch to 24.04-2.x daily update channel) and try again!")
                wrapMode: Text.Wrap
            }
    ...
    Component {
        ...
        Dialog {
            ...
            Label {
                text: i18n.tr("Waydroid doesn't support OTAs for Android versions past 13 just yet. Your device can still run unofficial Android 16 images (with some additional bugs) from <a href=\"https://volla.tech/filedump/waydroid-16\">https://volla.tech/filedump/waydroid-16</a> which Waydroid Helper can setup (excluding GAPPS). <br><br> Keep in mind currently you must also add the \"Waydroid Stop app\" icon to your drawer; Waydroid sessions are still left running in the background despite the window being visually closed, and the UI cannot be brought back up conveniently otherwise afterward. <br><br> Do you want to continue?")
                wrapMode: Text.Wrap
                onLinkActivated: Qt.openUrlExternally(link)
            }
